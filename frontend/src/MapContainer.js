...
export default class MapContainer extends Component {
  constructor() {
    ...
    this.createFilteredAccidentMarkers = this.createFilteredAccidentMarkers.bind(
      this
    );
    this.createFilteredTheftMarkers = this.createFilteredTheftMarkers.bind(
      this
    );
  ...
  createFilteredTheftMarkers() {
    ...
    if (this.state.theftYear && this.state.theftYearChoices.length > 0) {
      theftMarkers = theftMarkers.filter((marker) => {
        return this.state.theftYearChoices.includes(
          marker.occurrenceYear.toString()
        );
      });
    }
    if (
      this.state.theftNeighbourhood &&
      this.state.theftNeighbourhoodChoices.length > 0
    ) {
      theftMarkers = theftMarkers.filter((marker) => {
        let neighbourhood = marker.neighbourhood.substring(
          0,
          marker.neighbourhood.indexOf("(") - 1
        );
    ...
    if (
      this.state.theftBikeType &&
      this.state.theftBikeTypeChoices.length > 0
    ) {
      theftMarkers = theftMarkers.filter((marker) => {
  ...
  createFilteredAccidentMarkers() {
    ...
    if (this.state.accidentYear && this.state.accidentYearChoices.length > 0) {
      accidentMarkers = accidentMarkers.filter((marker) => {
        if (
          this.state.accidentYearChoices.includes(marker.date.substring(0, 4))
        ) {
          return true;
        }
      });
    ...
    if (
      ...
      this.state.accidentNeighbourhoodChoices.length > 0
    ) {
      accidentMarkers = accidentMarkers.filter((marker) => {
        if (marker.source == "User Submitted Data") {
          return this.state.accidentNeighbourhoodChoices.includes(
    ...
    if (
      this.state.roadCondition &&
      this.state.roadConditionChoices.length > 0
    ) {
      accidentMarkers = accidentMarkers.filter((marker) => {
    ...
    if (
      this.state.lightCondition &&
      this.state.lightConditionChoices.length > 0
    ) {
      accidentMarkers = accidentMarkers.filter((marker) => {
    ...
    if (
      this.state.accidentSource &&
      this.state.accidentSourceChoices.length > 0
    ) {
      accidentMarkers = accidentMarkers.filter((marker) => {
